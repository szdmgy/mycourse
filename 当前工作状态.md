...
## 下一步：阶段 8 — 管理员页面 + 导入预览前端

**目标**：管理员页面继承 base.html + 导入预览改用 base.html

需要做的工作（参见 `开发计划.md` 阶段 8）：
- 8.1 `manager.html` 继承 base.html
- 8.2 `userList.html` 继承 base.html
- 8.3 `upload_files.html` 重构：增加预览确认步骤
- 8.4 `import_preview.html` 改为继承 base.html

---

## 待办/待确认事项

- 阶段 0~7 全部完成，尚未进行完整的端到端测试
- `import_preview.html` 使用独立样式，阶段 8 应改为继承 base.html
- Bootstrap 3 文件仍保留在 static/ 目录（旧模板文件保留在磁盘上但无视图引用），阶段 9 统一清理
- 旧教师端模板（teacherCourseChange.html、teacherTasks.html）已被 teacherCourseDetail.html 替代，阶段 9 删除
- 旧 JS 文件 teacherTasks.js 已不再被新模板引用（新页面 JS 全部内联），阶段 9 清理
- 用户已明确授权直接实施
