...
**目标**：创建 base.html 公共模板、统一 Bootstrap 5、全局大字体

需要做的工作（参见 `开发计划.md` 阶段 5）：
- 5.1 创建 `base.html` 公共模板（导航栏 + 页面框架 + 全局字体）
- 5.2 设定全局字体规范（body 20px, 表格 18px, h1 32px/h2 28px/h3 24px）
- 5.3 创建 `static/css/global.css` 全局样式
- 5.4 移除 Bootstrap 3 相关文件
- 5.5~5.9 login/change_password/profile 等页面适配

---

## 待办/待确认事项

- 阶段 3~4 后端改造完成，等待用户测试确认
- 前端模板尚未全面适配新返回数据（将在阶段 5~8 处理）
- import_preview.html 使用 Bootstrap 5 独立样式，后续 base.html 建好后可改为继承
...
- 时间：2026-02-22
- 内容：完成阶段 4 导入预览确认流程（upload_data.py 三层拆分 + 2 个新视图 + 预览模板）
- 最新 commit：`76d69cc`（阶段 4 代码尚未提交）
