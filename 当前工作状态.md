...
|------|------|------|
| 阶段 0–9 | 见下文历史节点 | ✅ 已完成 |
| 一期增强 | 时间戳/API Key/身份切换、定性批改、报告模板、预检、在线预览 | ✅ 已实现（待/正在推送） |
| 2026-07-21 | 批改可选参考分 `HomeworkGrade.score`（0–100，仅教师） | ✅ 已完成 |

### 最近一次交付（2026-07-21）

- 模型字段 `score` + 迁移 `0011_homeworkgrade_score`
- 批改页等级旁输入参考分；单条/批量保存；Excel 明细列；API 可选 `score`
- 文档：教师版批改专节、学生版成绩可见性、README / API / 部署速查 / 变更记录
...
4. **单附件**：每个作业只允许 1 个附件（简化方案），文件命名规则：`标题_学号_姓名.扩展名`，如需多文件可打包为 ZIP
...
Task 模型字段:
  ...
  fileType (允许的文件类型, 如 .docx,.pdf 或 * 不限)

Homework 模型字段:
  user(FK→UserProfile), task(FK→Task), time(auto_now)

HomeworkFile 模型字段:
  homework(OneToOne→Homework), filePath(255), originalName
  每个作业只允许一个附件
```
...
将多附件（1~3 slot）简化为每作业 1 个附件，大幅降低系统复杂度：

**模型变更：**
- Task：删除 `maxFiles`, `slot1Name/Type`, `slot2Name/Type`, `slot3Name/Type`，新增 `fileType`（允许的文件类型）
- HomeworkFile：删除 `slot` 字段，`homework` 从 ForeignKey 改为 OneToOneField
- 生成迁移 `0002_remove_homeworkfile_unique_homework_slot_and_more`，重建数据库

**文件名规则：**
- 保存格式：`标题_学号_姓名.扩展名`
- `safe_filename()` 增强：额外过滤 `<>:"|?*` 及控制字符

**影响文件（11 个）：**
- `models.py` — 字段精简
- `views.py` — 15+ 处：上传/下载/统计/表单/复制/历史 API 全部简化
- `upload_data.py` — Excel 格式简化为 4 列（标题/内容/文件类型/显示）
- `admin.py` — list_display 精简
- `studentSubmit.html` — 单文件上传界面
- `studentTaskList.html` — 去掉 partial 状态
- `teacherCourseDetail.html` — 去掉 slot 配置/toggleSlots JS/附件数列
- `taskChange.html` — 去掉 slot 编辑区域
- `homeworkRecords.html` — 附件列简化为 已上传/无文件
- `delayRecords.html` — 去掉部分提交分支
- `global.css` — 移除 .badge-partial

---

## 待办/待确认事项

- 建议用真实数据做一次完整端到端测试（导入 Excel → 教师添加实验 → 学生提交 → 教师下载）
...
- 时间：2026-02-22
- 内容：单附件简化重构（多 slot → 单文件，文件名 标题_学号_姓名.ext）
- 最新 commit：待提交
- Git 状态：有未提交改动
