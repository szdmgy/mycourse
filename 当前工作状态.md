...
**目标**：重构数据导入为"解析 → 预览 → 确认 → 写入"

需要做的工作（参见 `开发计划.md` 阶段 4）：
- 4.1 重构 `upload_data.py`：拆分为解析函数和写入函数
- 4.2 新增 `preview_import` 视图：解析文件并返回预览数据
- 4.3 新增 `confirm_import` 视图：确认后执行写入
- 4.4 预览数据存 session（或临时文件）供确认时使用
- 4.5 新增 `import_preview.html` 模板
- 4.6 更新路由
...
- 阶段 3 后端改造完成，等待用户测试确认
- 前端模板尚未适配阶段 3 新返回数据（将在阶段 5~7 处理）
...
- 时间：2026-02-22
- 内容：完成阶段 3 全部 10 个子任务（views.py + urls.py），Django check 通过
- 最新 commit：`784a7e7`（阶段 3 代码尚未提交）
