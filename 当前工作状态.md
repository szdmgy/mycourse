...
**目标**：清理废弃文件、移除 Bootstrap 3、回归测试

需要做的工作（参见 `开发计划.md` 阶段 9）：
- 9.1 移除废弃模板（login2.html、teacher.html、test.html、副本文件等）
- 9.2 移除 Bootstrap 3 CSS/JS 文件
- 9.3 移除旧版教师页面（teacherCourseChange.html、teacherTasks.html）
...
- 阶段 0~8 全部完成，尚未进行完整的端到端测试
- Bootstrap 3 文件仍保留在 static/ 目录，阶段 9 统一清理
- 旧模板文件（teacherCourseChange.html、teacherTasks.html、login2.html、teacher.html、test.html 等）仍在磁盘上但无视图引用，阶段 9 删除
- 旧 JS/CSS 文件（teacherTasks.js、manager.js、manager.css 等）不再被新模板引用，阶段 9 清理
- 用户已明确授权直接实施
...
- 时间：2026-02-22
- 内容：完成阶段 8 管理员页面 + 导入预览前端，阶段 0~8 全部完成
- 最新 commit：待提交
- Git 状态：有未提交改动
